...
Since release 1.3 Minecraft supports a method of connectivity called 'SRV' Record, in principle they allow you to bind a hostname such as `mycoolserver.servers.fun` to an ip and a random port. The difference when using an SRV record over an A record is if you have a non standard port e.g. 25570 for your Minecraft server an A record would require you to type `mycoolserver.servers.fun:25570` into your client.

In using an SRV record you bind the hostname and the service you're running (a Minecraft server) to the specific port so when you go to your client you would only have to type `mycoolserver.servers.fun` into your client and the SRV record would tell your client "hey use this port".
...
<Image src="https://cdn.setup.md/Screenshot%202025-10-19%20at%2016.43.23.png" alt="Cloudflare SRV Record" width="750px" height="750px" />
