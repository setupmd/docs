...
<LinkCard
  title="Ngrok"
  description="ngrok simplifies app delivery by unifying API gateway, Kubernetes ingress, multi-cluster load balancing and more with ngrok&#x27;s Universal Gateway."
...
This can be a downside for those who want [signed chat](/extra/signed-chat) to be enforced on their servers but want to advertise and grow their playerbase on the network.
:::

<LinkCard
  title="Minekube Connect"
  description='The Ingress Tunnel for Minecraft Servers'
