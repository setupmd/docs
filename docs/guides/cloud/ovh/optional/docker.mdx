...
We utilise Arc for patch maintenance, if you already have an RMM this functionality is already most likely available to you and you should use that instead.

To automate patch management you firstly need to sign up for an Azure account [here](https://azure.microsoft.com/account/sign_up). Once the sign up process has been completed the remaining steps should be completed from within the Azure portal.