...
Unfortunately, the following steps will need to be created on a per host level, we now need to create a virtual adaptor that sits on the OVH vRACK MAC Address. This will allow us for across host communication within our cluster but without going over the public internet.
...
- Select 'Linux Bridge'
  - Name - 'vmbr1'
  - Autostart - 'Tick'
  - Bridge Ports - '{second network adaptor name here that isn't already bridged}'
- Select 'Create'
